    ...
    <div class="first-screen-center" style="height: 100%">
      <div class="first-screen-center-info" style="height: 100%">
        <div class="leftBox">
          ...
          <div v-if="isA==true" class="leftTable">
            <el-table
            ...
            >
              ...
              <el-table-column
                ...
                label="有效值"
              >
                <template slot-scope="scope"> <span>{{ fun(scope.row.effectiveValue) }}</span></template></el-table-column>
            </el-table>
            <el-pagination
              v-if="total>0"
              class="renderPage"
          ...
          <div class="chartsMain">
            <div v-if="isA" class="partBox">
              <div v-if="lineData.length>0" style="width: 100%;">
                ...
                <div style="margin-bottom: 15px">测点路径：{{ treeObj.pppidLabel }} \ {{ treeObj.ppidLabel }} \ {{ treeObj.pidLabel }} \ {{ treeObj.label }} \ 采样时间：{{ cjDate }} \ 游标：<span class="subText">{{ lineToTipObj.xLabel }} \ 有效值:{{ fun(lineToTipObj.yValue) }}</span></div>
              </div>
            ...
            <div v-if="isB" class="partBox">
              ...
              <div v-if="lineData2.length>0" style="margin-bottom: 15px">测点路径：{{ treeObj.pppidLabel }} \ {{ treeObj.ppidLabel }} \ {{ treeObj.pidLabel }} \ {{ treeObj.label }}\ 采样时间：{{ cjDate }} \ 游标：<span class="subText">{{ flineToTipObj2.xLabel }} \有效值: {{ fun(lineToTipObj2.yValue) }}</span></div>
            ...
            <div v-if="isC" class="partBox">
              ...
              <div v-if="lineData3.length>0" style="margin-bottom: 15px">测点路径：{{ treeObj.pppidLabel }} \ {{ treeObj.ppidLabel }} \ {{ treeObj.pidLabel }} \ {{ treeObj.label }} \ 采样时间：{{ cjDate }} \ 游标：<span class="subText">{{ lineToTipObj3.xLabel }} \ 有效值:{{ fun(lineToTipObj3.yValue) }}</span></div>
            ...
            <div v-if="isD" class="partBox">
              ...
              <div v-if="lineData4.length>0" style="margin-bottom: 15px">测点路径：{{ treeObj.pppidLabel }} \ {{ treeObj.ppidLabel }} \ {{ treeObj.pidLabel }} \ {{ treeObj.label }} \ 采样时间：{{ cjDate }} \ 游标：<span class="subText">{{ lineToTipObj4.xLabel }} \ 有效值:{{ fun(lineToTipObj4.yValue) }}</span></div>
...
export default {
  ...
  methods: {
    fun(val) { return Number(val).toFixed(2) },
    initGet() {
      if (this.isA == true) {
        this.getCharts()
      }
    ...
    handleClick(data) {
      ...
      this.treeObj.pidLabel = data.pidLabel
      this.treeObj.label = data.label
      const dataType = data.value.split('_')[1]
    ...
    getCharts() {
      ...
      this.$api.analysis(params).then(response => {
        if (response.code === '0') {
          ...
          this.newCreatime = response.data[0].createTime
          console.log('newCreatime',this.newCreatime)
          // this.lineData = response.data
          const dataArr = response.data
          const jsonArr = JSON.parse(JSON.stringify(dataArr).replace(/createTime/g, 'name').replace(/effectiveValue/g, 'value'))
          const arr = [
            {
              name: '',
              data: jsonArr
    ...
    getCharts2() {
      const params = {
        algorithmId: 33,
        createTime: this.newCreatime, // this.cjDate,
        equipmentId: this.treeCilckId,
